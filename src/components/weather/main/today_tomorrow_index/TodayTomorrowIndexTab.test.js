...
describe('TodayTomorrowIndexTabコンポーネント', () => {
    it('プロップスのテスト', () => {
        ...
        const wrapper = shallow(<TodayTomorrowIndexTab data={data} />);

        const todayTomorrowIndexTabNode = wrapper.find('div.TodayTomorrowIndexTab');
        expect(todayTomorrowIndexTabNode).toHaveLength(data.length);
        for (var idx = 0; idx < data.length; idx++) {
            expect(todayTomorrowIndexTabNode.at(idx).text()).toContain(data[idx].month + '月');
            expect(todayTomorrowIndexTabNode.at(idx).text()).toContain(data[idx].day + '日');
            const dayOfWeekdStr = getDayOfWeekStr(
                data[idx].year,
                data[idx].month,
                data[idx].day);
                expect(todayTomorrowIndexTabNode.at(idx).text()).toContain('（' + dayOfWeekdStr + '）');
        }
    });
});
